...
export const getSocketToken = async (req, res) => {
  try {
    // Generate a socket token with the user ID for socket authentication
    const socketToken = jwt.sign({ userId: req.user._id }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: "1h" })
